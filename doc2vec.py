...
def load_glove(filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            dic[word[0]] = np.array(word[1:], dtype=float)
    return dic
...
def distances(article, articles):
    ...
    for a in articles:
        if a == article:
            continue
        dis = np.linalg.norm(article[3] - a[3])
        distance.append((dis, a))
    return distance
...
def recommended(article, articles, n):
    ...
    distance.sort()
    closest_words = [d[1] for d in distance]
